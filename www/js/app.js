...
$(function() {
    ...
    $('body').on('click', '.item', function() {
        ...
        if(go == 'login') {
           cordovaOauthLogin();
            ...
            $.ajax({
                url: '/user/inventory_change/' + inv_appid  + '/' + getCookie('user_id_reg'),
                method: 'GET',
            ...
            window.location.href = '/logout/' + refresh_token;
    ...
    $('body').on('click', '#tradesSent', function() {
        ...
        $.ajax({
            url: '/user/sent_offers/' + getCookie('user_id_reg'),
            method: 'GET',
            success: function(data) {
                if(data.success) {
    ...
    $('body').on('click', '#tradesReceived', function() {
        ...
        $.ajax({
            url: '/user/received_offers/' + getCookie('user_id_reg'),
            method: 'GET',
            success: function(data) {
                if(data.success) {
    ...
    $('body').on('click', '.gameChange', function() {
        ...
        if(inventory == false) {
            ...
            $.ajax({
                url: '/user/change_game/' + $app_id + '/' + trade_uid + '/' + getCookie('user_id_reg'),
                method: 'GET',
                success: function(data) {
                    if(data.success) {
            ...
            $.ajax({
                url: '/user/inventory_change/' + $app_id  + '/' + getCookie('user_id_reg'),
                method: 'GET',
                success: function(data) {
                    if(data.success) {
                        var my_value = 0;
    ...
    if(window.location.pathname == "/auth/opskins") {
        cordovaOauthLogin();
    }
    ...
    $('body').on('click', '#withdrawto_opskins', function() {
        ...
        $.ajax({
            url: '/user/items_to_opskins/' + encodeURIComponent(opskins_selected.join(','))  + '/' + getCookie('user_id_reg'),
            method: 'GET',
            success: function(data) {
                if(data.success) {
    ...
    $('body').on('click', '.view_offer', function() {
        ...
        $.ajax({
            url: '/user/get_trade/' + $trade_id  + '/' + getCookie('user_id_reg'),
            method: 'GET',
            success: function(data) {
                if(data.success) {
    ...
    $('body').on('click', '#trade_accept_trade_', function() {
        ...
        $.ajax({
            url: '/user/accept_trade/' + $trade_id + '/' + $2fa  + '/' + getCookie('user_id_reg'),
            method: 'GET',
            success: function(data) {
                if(data.success) {
    ...
    $('body').on('click', '#trade_decline_trade', function() {
        ...
        $.ajax({
            url: '/user/decline_trade/' + $trade_id  + '/' + getCookie('user_id_reg'),
            method: 'GET',
            success: function(data) {
                if(data.success) {
    ...
    $('body').on('click', '.makeOfferFriend', function() {
        ...
        if($trade.length >= 17) {
            $.ajax({
                url: '/user/trade/' + encodeURIComponent($trade)  + '/' + getCookie('user_id_reg'),
                method: 'GET',
                success: function(data) {
                    if(data.success) {
                        var user = data.user;
    ...
    if(window.location.pathname == "/user/want_login") {
        var parts = window.location.search.substr(1).split("&");
        var $_GET = {};
        for (var i = 0; i < parts.length; i++) {
            var temp = parts[i].split("=");
        ...
        setCookie('access_token', $_GET['access_token'], 365);
        setCookie('refresh_token', $_GET['refresh_token'], 365);

        $.ajax({
            url: '/user/resume_login/' + encodeURIComponent($_GET['refresh_token']) + '/' + encodeURIComponent($_GET['access_token']) + '/' + getCookie('user_id_reg'),
            method: 'GET',
            success: function(data) {
                if(data.success) {
                    window.location.href = '/';
    ...
    $('body').on('click', '#deleteFriends', function() {
        ...
        for(var i = 0; i < count; i++) {
            setCookie('friends_' + i, '', 0);
        }

        setCookie('all_friends', '', 0);
    ...
    $('body').on('click', '#tradeItemsNow', function() {
        ...
        if(selected_items_to_send.length == 0) items_to_be_sended = 0;
        if(selected_items_to_receive.length == 0) items_to_be_received = 0;

        if(fconcated.length == 0) return $.ambiance({message: 'You can\'t send an empty offer!', type: 'error'});
        $.ajax({
            url: '/user/' + encodeURIComponent($('#trade2FAcode').val()) + '/' + encodeURIComponent($msg_trade) + '/make_trade/' + encodeURIComponent(items_to_be_sended) + '/' + encodeURIComponent(items_to_be_received) + '/' + encodeURIComponent(trade_user)  + '/' + getCookie('user_id_reg'),
            method: 'GET',
            success: function(data) {
                if(data.success) {
...
function checkUser() {
    ...
    $.ajax({
        url: '/user/resume_login/' + encodeURIComponent(refresh_token) + '/' + encodeURIComponent(access_token) + '/' + getCookie('user_id_reg'),
        method: 'GET',
        success: function(data) {
            if(data.success) {
                ...
                checkingToken(data.time_left);

                setCookie('access_token', data.access_token, 365);
                setCookie('refresh_token', data.refresh_token, 365);
...
function addNewFriend(avatar, name, trade_link) {
    ...
    var all_friends = getCookie('all_friends').split('<||>');
    ...
    if(already_friend == 0) {
        setCookie('friends_' + count, encodeURIComponent(avatar) + "|" + encodeURIComponent(name) + "|" + encodeURIComponent(trade_link), 365);

        if(getCookie('all_friends') == "") {
            setCookie('all_friends', name, 365);
        } else {
            var all_friends = getCookie('all_friends');
            setCookie('all_friends', all_friends+"<||>"+name, 365);
        }
    }
}
...
function showFriendsToTrade() {
    ...
    for(var i in cooks) {
     ...
     if(getCookie('all_friends') != '') $delete = '<h5><button type="button" class="btn" id="deleteFriends" style=" background-color: #a62626; "><i class="fas fa-user-slash"></i>&nbsp;DELETE FRIENDS</button></h5>';
...
function cordovaOauthLogin() {
    ...
    if(getCookie('user_id_reg') == "") setCookie('user_id_reg', p, 365555);

    window.open(
        'https://oauth.opskins.com/v1/authorize/?client_id=df9a38af12d4&state=' + getCookie('user_id_reg') + '&duration=permanent&mobile=1&scope=identity+trades+items+manage_items&response_type=code',
        '_self',
        'location=no'
    );
}
...
function checkingToken(timeleft) {
    var time = timeleft;
